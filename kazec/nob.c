...
#define BUILD_FOLDER "./kazec/build/"
...
int main(int argc, char **argv) {
    ...
    if(!nob_walk_dir("./kazec/src", visit_src, .data = &procs)) return 1;
    if(!nob_procs_flush(&procs)) return 1;

    if(!nob_walk_dir("./kazec/src", visit_move_obj, .data = &procs)) return 1;
    if(!nob_procs_flush(&procs)) return 1;
    ...
    nob_cc_flags(&cmd);
    nob_cmd_append(&cmd, "-c");
    nob_cc_inputs(&cmd, "./kazec/main.c");
